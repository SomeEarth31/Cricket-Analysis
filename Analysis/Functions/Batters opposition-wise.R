bat.against <- function(number){
  ...
  bat.opposition <- BATTERS[2]$Against[rank] # opposition teams
  
  cat("Stats of", BATTERS$Name[number], "opposition-wise \n")
  print(bat.opposition)
  ...
  for(i in 1:nrow(bat.opposition)){
    ...
    avg_value <- suppressWarnings(as.numeric(bat.opposition[[i, 4]]))
    inns_value <- suppressWarnings(as.numeric(bat.opposition[[i, 2]]))
    no_value <- suppressWarnings(as.numeric(bat.opposition[[i, 3]]))
    
    if(bat.opposition[[i, 1]] %in% strong.teams){
      # Only proceed if avg is not NA
      if(!is.na(avg_value)){
        cat("Avg of", BATTERS$Name[number], "against", bat.opposition[[i, 1]], "is", avg_value, "\n")
  ...
  if(!is.na(strong.inns) && strong.inns > 0){
    ...
    cat("Overall average of", BATTERS$Name[number], "against strong teams is", strong.avg, "\n")
  } else {
  ...
  for(i in 1:nrow(bat.opposition)){
    ...
    if(bat.opposition[[i, 1]] %in% weak.teams){
      if(!is.na(avg_value)){
        cat("Avg of", BATTERS$Name[number], "against", bat.opposition[[i, 1]], "is", avg_value, "\n")
  ...
  if(!is.na(weak.inns) && weak.inns > 0){
    ...
    cat("Overall average of", BATTERS$Name[number], "against weak teams is", weak.avg, "\n")
  } else {
    cat(BATTERS$Name[number], "has not batted or insufficient data against weak teams\n")
    weak.avg = NA
  }
...
save(Bat_opp, file = "../Bat_opp.RData")