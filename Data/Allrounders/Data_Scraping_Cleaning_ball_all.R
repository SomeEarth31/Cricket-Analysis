...
# and the index of the stats that we require(index of the table)
clean_bat = function(web_link, ind, photo_link) 
{
  ...
  if(p_type == "Allrounder" | p_type == "Batting Allrounder" | p_type == "Batters" | p_type == "Top order Batter")
  {
  #Call Python function here, that generated the RData file. 
  system(paste("conda run -n Webscrape python Syn_Scrape_ball_all.py", web_link, ind))
  ...
  team_w = all_tables[[2]][c(1,7,8,11)] 
  ...
  country_w = all_tables[[3]][c(1,7,8,11)] 
...
#This works!!
#clean_bat('https://www.espncricinfo.com/cricketers/ravindra-jadeja-234675/bowling-batting-stats')
...
data_gen = function(players)
{
  ...
  for(p in players)
  {
    ...
    names(out_clean$Teams)[2] = paste(name,"Runs")
    names(out_clean$Teams)[3] = paste(name,"Wkts")
    names(out_clean$Teams)[4] = paste(name,"Avg")
    
    names(out_clean$Country)[2] = paste(name, "Runs")
    names(out_clean$Country)[3] = paste(name, "Wkts")
...
ARS_ball = data_gen(all_rounders)

save(ARS_ball, file = "../Allrounders_ball.RData")